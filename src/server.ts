    ...
    const answers = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          colors.bgRed.black('❌ Exit') // Red background, black text
        ],
      },
    ]);
...
// view all employees (orange)
const viewEmployees = async () => {
    const result = await pool.query('SELECT * FROM employee');
    console.log(chalk.hex('#FF9500')('All Employees:'));
    console.table(result.rows);
  };
...
// add an employee (blue)
const addEmployee = async () => {
  console.log(colors.blue('Add an Employee:'));
...
// add a role (purple)
const addRole = async () => {
  console.log(chalk.hex('#AF52DE')('Add a Role:'));
